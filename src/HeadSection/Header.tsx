...
import { BsGlobe, BsChatLeft } from "react-icons/bs";
...
import { RiGroupLine, RiBuildingLine, RiPlayListAddFill } from "react-icons/ri";
import { MdOutlineKeyboardArrowDown } from "react-icons/md";

// import { IoIosClose } from "react-icons/io";
import SellerFlowSteps from '../SellerFlowSteps/sellerFlow'
import './Header.css'
...
function ResponsiveAppBar(props: any) {
  ...
  const searchObject = { name: searchInput, id: uuidv4() }

  const [searchOpen, setSearchOpen] = React.useState(false);
  const [options, setOptions] = React.useState<readonly searchResults[]>([]);
  const loading = searchOpen && options.length === 0;

  const [accountLogoutStatus, setAccountLogoutStatus] = React.useState<boolean>(false)
  ...
  const addSearchResults = () => {    
    ...
    setSearchInput('')
    naviagete(`/SearchResults/${searchInput}`)
  }
  ...
  // maxWidth="xl" sx={{ backgroundColor: 'white' }}
  return (
    <>
      <form onSubmit={onSubmitForm} className='headercontainer'>
          ...
          <div className='top-inner-container'>
            ...
            <div className='top-inner-middle-container'>
              <div className='citys-dropdown-container'>
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => <TextField {...params} label="Dubai" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} className='coutryDropdownInput' />
              </div>
              {/*SearchBar*/}
            ...
            <div className='inner-end-container'>
              <div className='language-ads-container'>
                ...
                <button type="button" className='icon2' onClick={() => naviagete('/MyAds')} ><RiPlayListAddFill style={{ width: 20, height: 20 }} /><span className='lan-text'>MyAds</span></button>
              ...
              {accountLogoutStatus ? (<div className='signin-container'><button type="button" className="signin-signup-button" onClick={() => naviagete('/Login')}>SignIn/SignUp</button></div>) :
                (<div className='profile-details-main-container'>
                  <Tooltip title="Open Profile">
                    <IconButton onClick={handleOpenUserMenu} >
                      <div className='avatar-container'><span className='avatar-title'>S</span></div>
                    </IconButton>
                  </Tooltip>
                  <Menu
                    id="menu-appbar"
                  ...
                  >
                    {settings.map((each) => (
                      <MenuItem key={each.id} onClick={handleCloseUserMenu}>
                        <button type='button' className='profile-nav-buttons' onClick={() => onClickOnEachProfile(each.id)}>{each.name}</button>
                      </MenuItem>
                    ))}
                  </Menu>
                  <div className='avatar-name-container'>
                    <p className='profile-name'>Shanu</p>
                  </div>
                </div>)}
        ...
        <div className='header-bottom-container'>
          <div className='navigation-container'>
            {navItems.map((each)=>
              <button type='button' className='nav-butons-style'>{each.label} <MdOutlineKeyboardArrowDown/></button>
            )}
            {/* <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={motors}
              sx={{ width: 95 }}
              renderInput={(params) => <TextField {...params} label="Motors" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} />
            <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={motors}
              sx={{ width: 125 }}
              renderInput={(params) => <TextField {...params} label="Electronics" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} />
            <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={motors}
              sx={{ width: 125 }}
              renderInput={(params) => <TextField {...params} label="Classified" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} />
            <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={motors}
              sx={{ width: 95 }}
              renderInput={(params) => <TextField {...params} label="Jobs" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} />
            <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={motors}
              sx={{ width: 125 }}
              renderInput={(params) => <TextField {...params} label="Community" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} />
            <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={motors}
              sx={{ width: 105 }}
              renderInput={(params) => <TextField {...params} label="Property" sx={{ boxShadow: 'none', '.MuiOutlinedInput-notchedOutline': { border: 0 } }} />} /> */}
          </div>
        </div>
      </form>
    </>
